...
typedef struct AM_Instance
{
	...
	u16 currentStatVar ;
	SequenceNumber SN;
	struct pduLeft  pduLft;  //20140915加
...
{	  
 ...
 typedef struct um5_pduhdr  //20140915加
 {
	u8 FIfirst;
	u8 FIsecond;
	u8 E;
...
typedef struct RntiManag{
	...
	struct  TM_RX_Instance  ccchRecv; ///局部CCCH的管理；
...
typedef struct rlc_sv
{
	...
	long allocCode[MAX_CODE];             //保存未分配的code值
	struct pduLeft  pduLft;

	struct RntiManag rntiManag;
	struct  TM_TX_Instance  pcchIns; ///全局PCCH 和BCCH的管理；
