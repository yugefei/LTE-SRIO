...
struct rio_dma_para timer1_para = {0,0x76,0xfc200600,0x830afa00,0x8FC0,0,1000000,0x0,0};
...
void send_dsp_route_info_wrapper();
...
static void riodt_dbell_event(struct rio_mport *mport, void *dev_id, u16 sid, u16 tid, u16 info)
{
	...
	send_enb_type1(&g_riodt,0x76,dest_addr, 36800,0xfc200600,0);
	udelay(50);
	send_enb_type3(&g_riodt,0x76,0x830e5880, 256, 0xfc200400,0);
	sendcount++;
	db_cnt ++;
...
static enum hrtimer_restart hrtimer_handler3(struct hrtimer *timer) 
{
	...
	if(subframenum == 2 || subframenum == 7)
	{
		...
		udelay(100);
		send_enb_type3(&g_riodt,timer1_para.destid,0x830e5880,/*timer1_para.len*/256,/*timer1_para.local*/0xfc200400,(void *)timer1_para.arg);
		sendcount ++;
	}
	else 
	{
...
{
	timer1_para.port = 0;
	timer1_para.destid = 0x76;
	timer1_para.local = 0xfc200800;
	timer1_para.dest = 0x830afa00;
...
{
	int i;
	ENBULPHYADPtoPHYType1 Type1Public;
	ENB_UL_TYPE1_PUCCH_C Type1Pucchc;
	ENB_UL_TYPE1_PUSCH_C Type1Puschc;

	//TYPE1_PUBLIC
	Type1Public.GHead.NewFlag = 1; //u32
	Type1Public.GHead.SFN = sysframenum; //u16
	Type1Public.GHead.SubframeN = subframenum;
	Type1Public.UL_TYPE1_PUBLIC_C.ULVxxUserNum = 1;//u16
	Type1Public.UL_TYPE1_PUBLIC_C.padding = 0;//u16
	Type1Public.UL_TYPE1_PUBLIC_C.PUSCHOffset = sizeof(ENBULPHYADPtoPHYType1) + Type1Public.UL_TYPE1_PUBLIC_C.ULVxxUserNum * sizeof(ENB_UL_TYPE1_PUCCH_C);//u32
	Type1Public.UL_TYPE1_PUBLIC_C.PUCCHOffset = sizeof(ENBULPHYADPtoPHYType1);//u32
	memcpy(mapaddr + 0x100600,&Type1Public,sizeof(ENBULPHYADPtoPHYType1));
	//PUCCH_C
	for(i = 0;i < Type1Public.UL_TYPE1_PUBLIC_C.ULVxxUserNum;i ++)
	{
		...
		Type1Pucchc.NumofNP = 18;//u16
		Type1Pucchc.cqiInfoLen = 0;//u16
		Type1Pucchc.nPucchIndex = 0;//u16
		Type1Pucchc.detaPUCCHshift = 0;//u16
		...
		Type1Pucchc.padding = 0;//u16
		memcpy(mapaddr + 0x100600 + sizeof(ENBULPHYADPtoPHYType1) + sizeof(ENB_UL_TYPE1_PUCCH_C) * i,&Type1Pucchc,sizeof(ENB_UL_TYPE1_PUCCH_C));
	}
	//PUSCH_C
	for(i = 0;i < Type1Public.UL_TYPE1_PUBLIC_C.ULVxxUserNum;i ++)
	{
		Type1Puschc.RNTI = 61;//u16
		Type1Puschc.UlChSel = 0;//u16
		if(sysframenum == 0 && subframenum == 0)
			Type1Puschc.PGType = 0;//u16
		else
			Type1Puschc.PGType = 2;//u16
		Type1Puschc.NumofNP = 18;//u16
		if(Type1Puschc.RNTI == 67)
			Type1Puschc.PrbStart = 2;//u16
		else
		...
		Type1Puschc.UECategory = 4;//u16
		Type1Puschc.TM = 0;//u16
		Type1Puschc.harqFeedbackMode = 1;//u16
		...
		memcpy(mapaddr + 0x100600 + sizeof(ENBULPHYADPtoPHYType1) + Type1Public.UL_TYPE1_PUBLIC_C.ULVxxUserNum * sizeof(ENB_UL_TYPE1_PUCCH_C) + sizeof(ENB_UL_TYPE1_PUSCH_C) * i,&Type1Puschc,sizeof(ENB_UL_TYPE1_PUSCH_C));
	}
	   riodt_map_write(&(timer_dev->dma_dev[1]), 0x76, timer_data, 36800, 0xfc200600, 0);
}

static void send_enb_type3(struct riodt *timer_dev, u16 timer_destid, u64 timer_data, unsigned int timer_len, u64 timer_addr, void *timer_arg)
{
	ENBULPHYADPtoPHYType3 Type3;

	Type3.GHead.NewFlag = 1;//u32
	Type3.GHead.SetFlag.CmdFlag = 1;//u32
	Type3.UL_TYPE3_PUBLIC_C.PUSCHCOffset = sizeof(Type3.UL_TYPE3_PUBLIC_C);//u32
	Type3.UL_TYPE3_PUSCH_C.NumRbHo = 2;//u16
	Type3.UL_TYPE3_PUSCH_C.puschDeltaSS = 0;//u16
	Type3.UL_TYPE3_PUSCH_C.HoppingMode = 0;//u16
	Type3.UL_TYPE3_PUSCH_C.GroupHopEn = 0;//u16
	Type3.UL_TYPE3_PUSCH_C.SeqHopEn = 0;//u16
	Type3.UL_TYPE3_PUSCH_C.Dmrs1 = 0;//u16
	Type3.UL_TYPE3_PUSCH_C.CpType = 0;//u16
	Type3.UL_TYPE3_PUSCH_C.cellID = 0;//u16
	Type3.UL_TYPE3_PUSCH_C.NumPrbBw = 100;//u16
	Type3.UL_TYPE3_PUSCH_C.SFDirection = 2;//u16
	Type3.UL_TYPE3_PUSCH_C.SpecSFcfg = 0;//u16
	Type3.UL_TYPE3_PUSCH_C.SrsSubframConfig = 0;//u16
	Type3.UL_TYPE3_PUSCH_C.CSrs = 0;//u16
	Type3.UL_TYPE3_PUSCH_C.SrsMaxUpPts = 0;//u16
	memcpy(mapaddr + 0x100400,&Type3,sizeof(ENBULPHYADPtoPHYType3));
	riodt_map_write(&(timer_dev->dma_dev[1]), 0x76, 0x830e5880, sizeof(ENBULPHYADPtoPHYType3), 0xfc200400, 0);
}	
