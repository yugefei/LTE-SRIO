...
void complexRarPdu(void * ue_infor){
	...
	while(ue_numb>0){
		...
		from_len=from_len+len;	//ÒòÎªdataÖ¸ÕëÏòÇ°ÒÆ¶¯ÁËlen
		...
		ttl_len=ttl_len+len;
		/*if(newskb->tail-newskb->data!=len){	//Èç¹ûÊÇµÚÒ»´Î£¬from_len²»×ÔÔö£¬Ò²¾ÍÊÇfrom_lenÖ¸Ê¾µÄÊÇÒÑ¾­ÌîºÃµÄ×îºóÒ»¸ösduµÄ¿ªÊ¼Î»ÖÃ
...
void complexEnbMacPdu(FSM_PKT *skb){
	...
	if(typ_rnti==CRNTI_TYPE){
		...
		fsm_mem_cpy(skb->head,ici_msg,len);	//½«MAC TO RLCµÄICIÐÅÏ¢·ÅÖÃµ½SK_buffÖÐ
		...
		if(ce_ta){
			...
			if(tb_size>=len+sizeof(m_fixed_subhead)){
...
void setMACtoRLC_IciMsg(MACtoRLC_IciMsg *ici_msg,u32 lcid,u32 rnti){	//20140430¸Ä ÌîÐ´MACtoRLC_IciMsg
	FIN(setMACtoRLC_IciMsg());
...
void dealCcchSdu(FSM_PKT *skb,u32 offset){
	...
	if(SV(m_temp_cr)==NULL){
		SV(m_temp_cr)=(temp_contention_resolution *)fsm_mem_alloc(sizeof(temp_contention_resolution));	//20140922¸Ä
	}
...
u32 lteMacGetUlHeadLength(FSM_PKT *skb){
	...
	MAC_CE_Crnti *m_c_rnti=(MAC_CE_Crnti*)fsm_mem_alloc(sizeof(MAC_CE_Crnti));	//20140430¸Ä
...
void decomplexEndMacPdu(FSM_PKT *skb){
	...
	fsm_mem_set(m_c_rnti,0,sizeof(MAC_CE_Crnti));	//20140430¸Ä
	...
	SV_PTR_GET(mac_sv);
	head_len=lteMacGetUlHeadLength(skb);	//ÒÑ½âSDUµÄÎ»ÖÃ
	...
	//typ_rnti=C_RNTI;//²âÊÔÓÃµÄ	//FOR TEST
	...
	if(typ_rnti==CRNTI_TYPE){
		 ...
		 * IO_CTRL TO SRIO
		 */
		 //fsm_do_ioctrl(STRM_TO_SRIO, IOCCMD_MACtoSRIO_RA_Complete, NULL, 0);
	
	
	
		fsm_printf("[HEXI]DECOMPLEX C_RNTI\n");
		while(continue_flag==1){
			len=sizeof(MAC_SH_padding);		//»ñÈ¡LCID
		...
		if(lcid>=0&&lcid<11){
			...
			while(continue_flag==1){
				...
				if(f_domain==1){	//15bit×ÓÍ·		20140430¸Ä
					...
					setMACtoRLC_IciMsg(m_rlc_type2_icimsg,lcid,phy_ici->rnti);	//ÉèÖÃICIÏûÏ¢	
					...
					//sm_pkt_send(newskb,STRM_TO_RLC);
		...
		while(continue_flag){
			...
			if(f_domain==1){
				...
				fsm_pkt_send(newskb,STRM_TO_RLC);		//20140428 ¸Ä
		...
		fsm_mem_free(m_short_bsr);
		fsm_mem_free(m_long_bsr);
		fsm_mem_free(m_ue_infor_from_phy);
		fsm_pkt_destroy(skb);
		FOUT;
	}
