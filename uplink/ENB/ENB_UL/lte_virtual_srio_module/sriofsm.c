	...
	FSM_BLOCK_SWITCH
	{
		...
		FSM_STATE_UNFORCED(ST_IDLE, "IDLE",,)		
		{
			...
			{	
				...
				FSM_CASE_TRANSIT(6, ST_SEND_TYPE1_TO_SRIO, , "IDLE -> SEND_TYPE1_TO_SRIO")
...
static void type1_pkt_assemble(void){
	...
	if(pkptr != NULL)
	{
		...
		if (SV(stored_pkt_count) >= MAX_STORED_PKT){
			//fsm_printf("[ENB SRIO] stored_pkt_count %d FOUT.\n", SV(stored_pkt_count));
			FOUT;
		}
		SV(pkts[SV(stored_pkt_count)]) = pkptr;
		//fsm_printf("[ENB SRIO] packet %d has been stored.\n", SV(stored_pkt_count));
		SV(stored_pkt_count) ++;
	}
	FOUT;
}
...
static void type1_send_to_srio(){
	...
	send_data(0xfc100000 + (times%cache_num)*cache_size, pkptr->len, 0xfc300000);
	SV(stored_pkt_count) = 0;
	fsm_pkt_destroy(pkptr);
	times ++;
	}
...
static void packet_send_to_upperlayer(void)
{
	...
	if (SV(racomplete) == true){
		...
		else if(fsm_ntohl(sh_ptr->type) == 1)
		{
			...
			fsm_do_ioctrl(STRM_TO_MAC,IOCCMD_PHYtoMAC_RA_Req,(void *)Msg1,sizeof(u32)+sizeof(S_RAinfo));
			fsm_pkt_destroy(pkptr);
			test_send_ta();
		}
		else 
...
/**********************************************************/
	...
	ici_to_mac->tcid=2;
	ici_to_mac->rnti=61;
	ici_to_mac->ue_info.rnti = 61;
	ici_to_mac->ue_info.sfn = 1;
	ici_to_mac->ue_info.subframeN = 2;
	ici_to_mac->ue_info.crc = 0;
	ici_to_mac->ue_info.harqindex = 0;
	ici_to_mac->ue_info.harq_result = 0;
	ici_to_mac->ue_info.sr = 0;
	...
	ici_to_mac->ue_info.ta = 32;
	u16 data_len;
	data_len=*(u16 *)((u8 *)checkaddr+sizeof(ENBULPHYADPtoPHYType2)+sizeof(ENB_UL_TYPE2_PUCCH_C) + sizeof(ENB_UL_TYPE2_PUSCH_C) + sizeof(ENB_UL_TYPE2_PUSCH_D));
	pkptrtomac = fsm_pkt_create(2+data_len+sizeof(PHYadaptertoMAC_IciMsg));
