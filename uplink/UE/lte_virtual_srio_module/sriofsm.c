...
static void recv_tasklet_handler(unsigned long buffer,u16 pkt_count);
...
static void packet_send_to_eth(void)
{
	...
	********************************************************************************/
	static void packet_send_to_upperlayer(void)
	{
		...
		if (SV(racomplete) == true)
		{
			//printk("packet_send_to_upperlayer...\n");
			pkptr = fsm_pkt_get();
			sh_ptr = (struct lte_test_srio_head*)pkptr->data;
			...
			else
			{
				...
				else if(fsm_ntohl(sh_ptr->type) == 3)
				{
					...
					fsm_do_ioctrl(STRM_TO_MAC,IOCCMD_PDCCHtoMAC_ULGRANT,(void *)ulgrand_to_mac,sizeof(UEPHY_TO_MAC_ULgrant));
...
static void type2_recv_from_srio(void)
	...
	u16 pkt_count=0;
	int inter=0;
	flag = recieve_data(0xfc100000 + (times%cache_num)*cache_size,&pkt_count);
	if (flag == 1)
	{
		SV_PTR_GET(srio_sv);
		fsm_schedule_self(SV(recv_interval), _TYPE2_RECV_FROM_SRIO);
		recv_tasklet_handler((u32)(rtn_mmap() + (times%cache_num)*cache_size),pkt_count);
		times ++;
	}else{
		fsm_schedule_self(50, _TYPE2_RECV_FROM_SRIO);
		FOUT;
	}
}
...
static void type1_pkt_assemble_test(void)
{
	...
	//fsm_octets_print(pkptr->data,pkptr->len);
	//printk("subframenum:%d\n", subframenum);
	spin_lock(&UnionQueue[subframenum].my_lock);
	//printk("before copy...\n");
	fsm_mem_cpy((char *)(VirAddrStart[subframenum] + (UnionQueue[subframenum].NextData)*0x4000), pkptr->data, pkptr->len);
	...
	if(IOCTL_ARRIVAL)
	{
		switch(fsm_ev_ioctrl_cmd())
		{
			...
			case IOCCMD_RA_COMPLETE:
				...
				printk("racomplete = true\n");
				fsm_schedule_self(0, _TYPE2_RECV_FROM_SRIO);
				printk("SV(send_count) = %d\n",SV(send_count));
				SV(send_count) =0;
...
static void test_send_sf(void)
{
	...
	fsm_do_ioctrl(STRM_TO_MAC,IOCCMD_PHYtoMAC_SYSFRAME,(void *)Sf_info,sizeof(Sf_info));
...
static void recv_tasklet_handler(unsigned long buffer,u16 pkt_count){
	...
	PHYadaptertoMAC_IciMsg * ici_to_mac;
	ptr_from_srio = (void*)buffer; 
	SV_PTR_GET(srio_sv);
	do{

		type1head = (struct ENBPHYADPtoPHYType1*)ptr_from_srio;
		ptr_from_srio += sizeof(struct ENBPHYADPtoPHYType1);
		
		type1pbchc = (struct ENB_DL_TYPE1_PBCH_C*)ptr_from_srio;
		ptr_from_srio += sizeof(struct ENB_DL_TYPE1_PBCH_C);
		
		type1pbchd = (struct ENB_DL_TYPE1_PBCH_D*)ptr_from_srio;
		ptr_from_srio += sizeof(struct ENB_DL_TYPE1_PBCH_D);
		
		for (i = 0; i < 2; i ++){
			type1phichc = (struct ENB_DL_TYPE1_PHICH_C*)ptr_from_srio;
			ptr_from_srio += sizeof(struct ENB_DL_TYPE1_PHICH_C);
		}
		for (i = 0; i < 2; i ++){
			type1phichd = (struct ENB_DL_TYPE1_PHICH_D*)ptr_from_srio;
			ptr_from_srio += sizeof(struct ENB_DL_TYPE1_PHICH_D);
		}
		for (i = 0; i < 5; i ++){
			type1pdcchc = (struct ENB_DL_TYPE1_PDCCH_C*)ptr_from_srio;
			ptr_from_srio += sizeof(struct ENB_DL_TYPE1_PDCCH_C);
		}
		for (i = 0; i < 5; i ++){
			type1pdcchd = (struct ENB_DL_TYPE1_PDCCH_D*)ptr_from_srio;
			ptr_from_srio += sizeof(struct ENB_DL_TYPE1_PDCCH_D);
		}
		for (i = 0; i < 5; i ++){
			type1pdschc = (struct ENB_DL_TYPE1_PDSCH_C*)ptr_from_srio;
			ptr_from_srio += sizeof(struct ENB_DL_TYPE1_PDSCH_C);
		}
		for (i = 0; i < 5; i ++){
			type1pdschd = (struct ENB_DL_TYPE1_PDSCH_D*)ptr_from_srio;
			rnti = type1pdschd->RNTI;
			data_len = type1pdschd->PdschData;
			ptr_from_srio += sizeof(struct ENB_DL_TYPE1_PDSCH_D);
			pkt_no = *((int *)ptr_from_srio);			
			
			ptr_from_srio += sizeof(u32);
			if (i == 0){
				//printk("[ENB SRIO]tasklet   pkt_no = %d\n", pkt_no);
				pkptrtomac = fsm_pkt_create(data_len);
				fsm_mem_cpy(pkptrtomac->data,ptr_from_srio,data_len);
			}
			ptr_from_srio += data_len;
		}
		ici_to_mac=(PHYadaptertoMAC_IciMsg*)fsm_mem_alloc(sizeof(PHYadaptertoMAC_IciMsg));
		ici_to_mac->tcid=2;
		ici_to_mac->rnti=rnti;
		ici_to_mac->frameNo=7;
		ici_to_mac->subframeNo=2;
		fsm_mem_cpy(pkptrtomac->head,ici_to_mac,sizeof(PHYadaptertoMAC_IciMsg));
		if (rnti == 61){
			fsm_pkt_send(pkptrtomac, STRM_TO_MAC);
			pkt_recv++;
			SV(recv_pkt_from_enb)++;
			if(pkt_no==seq+1)
			{
			seq=pkt_no;
			}
	...
	while(rnti == 61&&(pkt_recv<pkt_count));
}
