...
static void ipadp_sv_init(void)
{
	...
	SV(packet_count) = 0;
	SV(interval) = 100000;
	//SV(V_Flag) = true;  //noted in 20150318
...
static void do_ioctl_period() 	//this function seems not in the state transmition.
{
...
static void packet_send_to_lowerlayer(void)
{
	...
	//struct UPDCP_IciMsg* Ici_ptr;  //time:20140729  noted in 20150318
	...
	switch(protocol){
		case 1:
			...
			//if(pkptr->transport_header != NULL){	//icmp_header?!
				...
				//if(SV(V_Flag) == true)		//noted in 20150318
				//{
