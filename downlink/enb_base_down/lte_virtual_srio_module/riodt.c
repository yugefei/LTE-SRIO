...
struct rio_dma_para timer1_para = {0,0x94,0xfc100800,0x81258000,0xfa0,0,1000000,0x0,0};
struct new_check_para timer2_para;
unsigned int sendcount = 0;
unsigned int totalcount = 0;
unsigned int offset = 0xfa0;
unsigned long long AddrStart[QUEUE_MAX_NUM] = { 0xfc100800+0*QUEUE_OFFSET*0xfa0,
...
unsigned int uesubframenum = 0;
unsigned int uesysframenum = 0;
...
static void send_enb_type1(struct riodt *timer_dev, u16 timer_destid, u64 timer_data, unsigned int timer_len, u64 timer_addr, void *timer_arg);
static void send_enb_type3(struct riodt *timer_dev, u16 timer_destid, u64 timer_data, unsigned int timer_len, u64 timer_addr, void *timer_arg);
...
int read_type2_mem()
{
	void *read_addr = mapaddr + 0x100800;
	unsigned int i;
	unsigned int offset = 0x8FC0;//每块缓存的大小，作为偏移量

	for(i = 0; i <= 5;i++)
	{
		if((*((u32 *)(read_addr + i*offset)) ==1)&&(*((u32 *)(read_addr + i*offset+68)) !=0))
		{
			//fsm_printf("type2 mem read %d\n",*((u32 *)(read_addr + i*offset+64)));
			//memread(unsigned long readaddr,unsigned int length)
			return i;
		}
	}
	return -1;
...
static void riodt_dbell_event(struct rio_mport *mport, void *dev_id, u16 sid, u16 tid, u16 info)
{
	...
	{
		subframenum = 5;
		sysframenum = (db_cnt*20000 - 1)%1024;
	}
...
int riodt_mmap()
{
	...
	/*VirAddrStart[QUEUE_MAX_NUM] = { mapaddr+0x800+0*QUEUE_OFFSET*0xfa0,
									mapaddr+0x800+1*QUEUE_OFFSET*0xfa0,
							        mapaddr+0x800+2*QUEUE_OFFSET*0xfa0,
									mapaddr+0x800+3*QUEUE_OFFSET*0xfa0,
									mapaddr+0x800+4*QUEUE_OFFSET*0xfa0,
									mapaddr+0x800+5*QUEUE_OFFSET*0xfa0,
									mapaddr+0x800+6*QUEUE_OFFSET*0xfa0,
									mapaddr+0x800+7*QUEUE_OFFSET*0xfa0,
									mapaddr+0x800+8*QUEUE_OFFSET*0xfa0,
									mapaddr+0x800+9*QUEUE_OFFSET*0xfa0
									};*/
	for(i=0;i<QUEUE_MAX_NUM; i++)
	{
		VirAddrStart[i]=mapaddr+0x800+i*QUEUE_OFFSET*0xfa0;
	}
...
static enum hrtimer_restart hrtimer_handler0(struct hrtimer *timer) 
{
	...
	u32 loca_addr = timer1_para.local;
	u32 dest_addr = 0x81258000; //0x812A2500  0x812A7400 
	...
	if(subframenum != 2 && subframenum != 7)
	{
		...
		{
			//printk("ENB subframenum = %u\n",subframenum);
			dest_addr = timer1_para.dest + offset * buffnum;
			buffnum ++;
			...
			riodt_map_write(&(g_riodt.dma_dev[1]), 0x94, dest_addr,4000, (AddrStart[subframenum]+CurPos*0xfa0), (void *)0);
			send_enb_type3(&g_riodt,0x94,0x8125CF00,256,0xfc100600,(void *)timer1_para.arg);
			//sendcount++;
			//printk("ENB sendcount = %u\n",sendcount);	
		}
...
static enum hrtimer_restart hrtimer_handler3(struct hrtimer *timer) 
{
	...
	prevtime = fsm_get_curtime();
	//printk("entering hrtimer_handler2...\n");
	//printk("buffnum = %u\n",buffnum);
	//printk("subframenum = %u\nsysframenum = %u\n",subframenum,sysframenum);
	//printk("dest_addr = %#x\n",	dest_addr);
	if(subframenum != 2 && subframenum != 7)
...
//		printk("It is a commen subframe\n");
		...
		//printk("before send hrtimer_handler2...\n");
		//printk("buffnum = %u\n",buffnum);
		//printk("subframenum = %u\nsysframenum = %u\n",subframenum,sysframenum);
		//printk("dest_addr = %#x\n",	dest_addr);
		send_enb_type1(&g_riodt,timer1_para.destid,dest_addr,/*timer1_para.len*/timer1_para.len,timer1_para.local,(void *)timer1_para.arg);
		udelay(100);
		send_enb_type3(&g_riodt,timer1_para.destid,0x8125CF00,/*timer1_para.len*/256,/*timer1_para.local*/0xfc100600,(void *)timer1_para.arg);
		sendcount ++;
	}
	else 
	{
		uplinkcount ++;
//		printk("It is a special subframe uplinkcount = %u\n",uplinkcount);
	}
...
//	printk("subframenum = %u\n",subframenum);
	...
	//printk("after send hrtimer_handler2...\n");
	//printk("buffnum = %u\n",buffnum);
	//printk("subframenum = %u\nsysframenum = %u\n",subframenum,sysframenum);
	//printk("dest_addr = %#x\n",	dest_addr);
	...
	if(sendcount == timer1_para.sendtimes)
	{
		...
		sendcount = 0;
		printk("subframenum = %u\nsysframenum = %u\nuplinkcount = %u\n",subframenum,sysframenum,uplinkcount);
		printk("dest_addr = %#x\n",	dest_addr);
		return HRTIMER_NORESTART;
	}
	else
...
static enum hrtimer_restart hrtimer_handler4(struct hrtimer *timer) 
{
	u32 dest_addr = timer1_para.dest;
	unsigned int offset = 0x2800;
	hrtimer_forward(timer, timer->base->get_time(), kt1);

//	nr_flag(&riodt, timer1_para.destid,timer1_para.dest,timer1_para.len,timer1_para.local,(void *)timer1_para.arg);
	...
	if(uesubframenum == 10)
	{
		uesubframenum = 0;
		uesysframenum++;
		if(uesysframenum == 1024)
			uesysframenum = 0;
	}
//	printk("dest_addr = %#x\n",	dest_addr);
	//uedl_type2(&g_riodt,timer1_para.destid,dest_addr,128,timer1_para.local,(void *)timer1_para.arg);
//	printk("UE type2 has been sent! dest_addr = %#x\n",	dest_addr);
...
static int rio_hrtimer1_start(struct riodt *dev, unsigned int destid)  //配置定时器
{
	...
	if(timer1_para.sendtimes == 0)
		...
		timer1.function = hrtimer_handler4;
	else
		timer1.function = hrtimer_handler3;
...
{
	timer1_para.port = 0;
	timer1_para.destid = 0x94;
	timer1_para.local = 0xfc100800;
	timer1_para.dest = 0x81258000;
	timer1_para.len = 0xfa0;
	timer1_para.seconds = 0;
	timer1_para.nseconds = 1000000;
	timer1_para.sendtimes =0;
	timer1_para.arg = 0;
	kt1 = ktime_set(timer1_para.seconds, timer1_para.nseconds); /*  sec,  nsec */
	hrtimer_init(&timer1, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	//hrtimer_start(&timer1, kt1, HRTIMER_MODE_REL);
...
static void send_dsp_route_info(struct riodt *timer_dev)
{
	...
	*/
	...
	route_info.AMC2_ = 0x50;
	route_info.DSP1_ = 0x94;
	route_info.DSP2_ = 0x96;
	route_info.FPGA1_ = 0x91;
	route_info.FPGA2_ = 0x92;
	
	memcpy(mapaddr + 0x300500, &route_info, sizeof(SRIO_ROUTE_INFO));
	//riodt_map_write(&(timer_dev->dma_dev[1]), timer_destid, timer_data, timer_len, timer_addr, timer_arg);
	riodt_map_write(&(timer_dev->dma_dev[1]), 0x94, 0x0c0ba100, 16, 0xfc400500, 0);
}
...
//	printk("type1head.GHead.SubframeN = %u\n",type1head.GHead.SubframeN);
...
//	printk("PBCH_C_Offset = %u %#x\n",type1head.DL_TYPE1_PUBLIC_C.PBCH_C_Offset,type1head.DL_TYPE1_PUBLIC_C.PBCH_C_Offset); 
	//PBCH_D内存开始位置
	type1head.DL_TYPE1_PUBLIC_C.PBCH_D_Offset = sizeof(ENBPHYADPtoPHYType1) + sizeof(ENB_DL_TYPE1_PBCH_C);
//	printk("PBCH_D_Offset = %u %#x\n",type1head.DL_TYPE1_PUBLIC_C.PBCH_D_Offset,type1head.DL_TYPE1_PUBLIC_C.PBCH_D_Offset);
	//PHICH_C内存开始位置
	type1head.DL_TYPE1_PUBLIC_C.PHICH_C_Offset = sizeof(ENBPHYADPtoPHYType1) + sizeof(ENB_DL_TYPE1_PBCH_C) + sizeof(ENB_DL_TYPE1_PBCH_D);
//	printk("PHICH_C_Offset = %u %#x\n",type1head.DL_TYPE1_PUBLIC_C.PHICH_C_Offset,type1head.DL_TYPE1_PUBLIC_C.PHICH_C_Offset);
	//PHICH_D内存开始位置
	type1head.DL_TYPE1_PUBLIC_C.PHICH_D_Offset = sizeof(ENBPHYADPtoPHYType1) + sizeof(ENB_DL_TYPE1_PBCH_C) + sizeof(ENB_DL_TYPE1_PBCH_D) + type1head.DL_TYPE1_PUBLIC_C.NumOfUEForPHICH * sizeof(ENB_DL_TYPE1_PHICH_C);
//	printk("PHICH_D_Offset = %u %#x\n",type1head.DL_TYPE1_PUBLIC_C.PHICH_D_Offset,type1head.DL_TYPE1_PUBLIC_C.PHICH_D_Offset);
	//PDCCH_C内存开始位置
	type1head.DL_TYPE1_PUBLIC_C.PDCCH_C_Offset = sizeof(ENBPHYADPtoPHYType1) + sizeof(ENB_DL_TYPE1_PBCH_C) + sizeof(ENB_DL_TYPE1_PBCH_D) + type1head.DL_TYPE1_PUBLIC_C.NumOfUEForPHICH * sizeof(ENB_DL_TYPE1_PHICH_C) + type1head.DL_TYPE1_PUBLIC_C.NumOfUEForPHICH * sizeof(ENB_DL_TYPE1_PHICH_D);
//	printk("PDCCH_C_Offset = %u %#x\n",type1head.DL_TYPE1_PUBLIC_C.PDCCH_C_Offset,type1head.DL_TYPE1_PUBLIC_C.PDCCH_C_Offset);
	//PDCCH_D内存开始位置
	type1head.DL_TYPE1_PUBLIC_C.PDCCH_D_Offset = sizeof(ENBPHYADPtoPHYType1) + sizeof(ENB_DL_TYPE1_PBCH_C) + sizeof(ENB_DL_TYPE1_PBCH_D) + type1head.DL_TYPE1_PUBLIC_C.NumOfUEForPHICH * sizeof(ENB_DL_TYPE1_PHICH_C) + type1head.DL_TYPE1_PUBLIC_C.NumOfUEForPHICH * sizeof(ENB_DL_TYPE1_PHICH_D) + type1head.DL_TYPE1_PUBLIC_C.PDCCHNum * sizeof(ENB_DL_TYPE1_PDCCH_C);
//	printk("PDCCH_D_Offset = %u %#x\n",type1head.DL_TYPE1_PUBLIC_C.PDCCH_D_Offset,type1head.DL_TYPE1_PUBLIC_C.PDCCH_D_Offset);
	//PDSCH_C内存开始位置
	type1head.DL_TYPE1_PUBLIC_C.PDSCH_C_Offset = sizeof(ENBPHYADPtoPHYType1) + sizeof(ENB_DL_TYPE1_PBCH_C) + sizeof(ENB_DL_TYPE1_PBCH_D) + type1head.DL_TYPE1_PUBLIC_C.NumOfUEForPHICH * sizeof(ENB_DL_TYPE1_PHICH_C) + type1head.DL_TYPE1_PUBLIC_C.NumOfUEForPHICH * sizeof(ENB_DL_TYPE1_PHICH_D) + type1head.DL_TYPE1_PUBLIC_C.PDCCHNum * sizeof(ENB_DL_TYPE1_PDCCH_C) + type1head.DL_TYPE1_PUBLIC_C.PDCCHNum * sizeof(ENB_DL_TYPE1_PDCCH_D);
//	printk("PDSCH_C_Offset = %u %#x\n",type1head.DL_TYPE1_PUBLIC_C.PDSCH_C_Offset,type1head.DL_TYPE1_PUBLIC_C.PDSCH_C_Offset);
	//PDSCH_D内存开始位置
	type1head.DL_TYPE1_PUBLIC_C.PDSCH_D_Offset = sizeof(ENBPHYADPtoPHYType1) + sizeof(ENB_DL_TYPE1_PBCH_C) + sizeof(ENB_DL_TYPE1_PBCH_D) + type1head.DL_TYPE1_PUBLIC_C.NumOfUEForPHICH * sizeof(ENB_DL_TYPE1_PHICH_C) + type1head.DL_TYPE1_PUBLIC_C.NumOfUEForPHICH * sizeof(ENB_DL_TYPE1_PHICH_D) + type1head.DL_TYPE1_PUBLIC_C.PDCCHNum * sizeof(ENB_DL_TYPE1_PDCCH_C) + type1head.DL_TYPE1_PUBLIC_C.PDCCHNum * sizeof(ENB_DL_TYPE1_PDCCH_D) + type1head.DL_TYPE1_PUBLIC_C.PDSCHNum * sizeof(ENB_DL_TYPE1_PDSCH_C);
//	printk("PDSCH_D_Offset = %u %#x\n",type1head.DL_TYPE1_PUBLIC_C.PDSCH_D_Offset,type1head.DL_TYPE1_PUBLIC_C.PDSCH_D_Offset);
	memcpy(mapaddr + 0x800,&type1head,sizeof(ENBPHYADPtoPHYType1));
	//PBCH_C参数
	type1pbchc.NumPrbBw = 100;
	type1pbchc.PhichDurtion = 0;
	type1pbchc.GPhichNg = 0;
	type1pbchc.spare = 0;
	memcpy(mapaddr + 0x800 + type1head.DL_TYPE1_PUBLIC_C.PBCH_C_Offset,&type1pbchc,sizeof(ENB_DL_TYPE1_PBCH_C));
	//PBCH_D参数
...
static void send_enb_type3(struct riodt *timer_dev, u16 timer_destid, u64 timer_data, unsigned int timer_len, u64 timer_addr, void *timer_arg)
{
	ENBPHYADPtoPHYType3 type3;
	type3.GHead.NewFlag = 1;
	//printk("NewFlag = %u\n",type3.GHead.NewFlag);
	type3.GHead.SetFlag.CmdFlag = 1;
	type3.DL_TYPE3_PUBLIC_C.PDSCHOffset = sizeof(GeneralHeader345) + sizeof(ENB_DL_TYPE3_PUBLIC_C) + sizeof(ENB_DL_TYPE3_PDCCH_C);
	//printk("PDSCHOffset = %u\n",type3.DL_TYPE3_PUBLIC_C.PDSCHOffset);
	type3.DL_TYPE3_PUBLIC_C.PDCCHOffset = sizeof(GeneralHeader345) + sizeof(ENB_DL_TYPE3_PUBLIC_C);
	//printk("PDCCHOffset = %u\n",type3.DL_TYPE3_PUBLIC_C.PDCCHOffset);
	type3.DLType3Pdcch_C.SpecSubFrmCfg = 2;
	type3.DLType3Pdsch_C.CellID = 0;
	type3.DLType3Pdsch_C.NumAntPort = 2;
	type3.DLType3Pdsch_C.CFI = 1;
	type3.DLType3Pdsch_C.CpType = 0;
	if(subframenum == 1 || subframenum == 6)
		type3.DLType3Pdsch_C.SFDirection = 1; //特殊frame
	else if(subframenum == 2 || subframenum == 7)
		type3.DLType3Pdsch_C.SFDirection = 2; //上行frame
	else
		type3.DLType3Pdsch_C.SFDirection = 3; //普通frame
		
	type3.DLType3Pdsch_C.PB = 1;
	memcpy(mapaddr + 0x600,&type3,sizeof(ENBPHYADPtoPHYType3)); //type3内存区位置0xfc100600
	riodt_map_write(&(timer_dev->dma_dev[1]), timer_destid, timer_data, timer_len, timer_addr, timer_arg);
	//printk("type3 send !\n");
}
/*
static void uedl_type2(struct riodt *timer_dev, u16 timer_destid, u64 timer_data, unsigned int timer_len, u64 timer_addr, void *timer_arg)
{
	...
	unsigned long long PDCCH_D = 0;
	UE_DL_TYPE2_PDSCH_C DL_TYPE2_PDSCH_C;
	void *PDSCH_D = NULL;

	//TYPE2 head
	uetype2.GHead.NewFlag = 1;//32bit
	uetype2.GHead.SFN = uesysframenum;//16bit
	uetype2.GHead.SubframeN = uesubframenum;//16bit
	uetype2.DL_TYPE2_PUBLIC_C.PDSCHLength = 32;//32bit
	uetype2.DL_TYPE2_PUBLIC_C.PDCCHOffset = sizeof(UEPHYADPtoPHYType2) + sizeof(DL_TYPE2_PBCH_D) + sizeof(DL_TYPE2_PHICH_D) + sizeof(DL_TYPE2_PDCCH_C);//32bit
	memcpy(mapaddr,&uetype2,sizeof(UEPHYADPtoPHYType2));
	//PBCH_D para
	DL_TYPE2_PBCH_D.SourcePBCH = 0;//32bit
	memcpy(mapaddr + sizeof(UEPHYADPtoPHYType2),&DL_TYPE2_PBCH_D,sizeof(DL_TYPE2_PBCH_D));
	//PHICH_D para
	DL_TYPE2_PHICH_D.HARQIndex = 1;//32bit
	memcpy(mapaddr + sizeof(UEPHYADPtoPHYType2) + sizeof(DL_TYPE2_PBCH_D),&DL_TYPE2_PHICH_D,sizeof(DL_TYPE2_PHICH_D));
	//PDCCH_C para
	DL_TYPE2_PDCCH_C.HARQprocessID = 0;//32bit
	DL_TYPE2_PDCCH_C.RV = 0;//32bit
	DL_TYPE2_PDCCH_C.NDI = 1;//16bit
	DL_TYPE2_PDCCH_C.padding = 0;//16bit
	memcpy(mapaddr + sizeof(UEPHYADPtoPHYType2) + sizeof(DL_TYPE2_PBCH_D) + sizeof(DL_TYPE2_PHICH_D),&DL_TYPE2_PDCCH_C,sizeof(DL_TYPE2_PDCCH_C));
	//PDCCH_D para 64bit
	PDCCH_D = 0xffffffffffffffff;
	memcpy(mapaddr + sizeof(UEPHYADPtoPHYType2) + sizeof(DL_TYPE2_PBCH_D) + sizeof(DL_TYPE2_PHICH_D) + sizeof(DL_TYPE2_PDCCH_C),&PDCCH_D,sizeof(PDCCH_D));
	//PDSCH_C para
	DL_TYPE2_PDSCH_C.CRC = 0x12345678;//32bit
	memcpy(mapaddr + sizeof(UEPHYADPtoPHYType2) + sizeof(DL_TYPE2_PBCH_D) + sizeof(DL_TYPE2_PHICH_D) + sizeof(DL_TYPE2_PDCCH_C) + sizeof(PDCCH_D),&DL_TYPE2_PDSCH_C,sizeof(DL_TYPE2_PDSCH_C));
	//PDSCH_D
	PDSCH_D = kmalloc(uetype2.DL_TYPE2_PUBLIC_C.PDSCHLength,GFP_ATOMIC);
	memset(PDSCH_D,1,uetype2.DL_TYPE2_PUBLIC_C.PDSCHLength);
	memcpy(mapaddr + sizeof(UEPHYADPtoPHYType2) + sizeof(DL_TYPE2_PBCH_D) + sizeof(DL_TYPE2_PHICH_D) + sizeof(DL_TYPE2_PDCCH_C) + sizeof(PDCCH_D) + sizeof(DL_TYPE2_PDSCH_C),PDSCH_D,uetype2.DL_TYPE2_PUBLIC_C.PDSCHLength);
	riodt_map_write(&(timer_dev->dma_dev[1]), timer_destid, timer_data, 128, timer_addr, timer_arg);
	kfree(PDSCH_D);
}
