...
MODULE_DESCRIPTION("lterrc enb kernel module");
MODULE_AUTHOR("");
MODULE_LICENSE("GPL");
...
#define SV_TYPE_NAME		rrc_sv_enb
...
{
	int fsm_id=-1;
	rrc_sv_enb_ue *sv_enb_ue = (rrc_sv_enb_ue *)fsm_mem_alloc(sizeof(rrc_sv_enb_ue));
	fsm_mem_set(sv_enb_ue, 0, sizeof(rrc_sv_enb_ue));		
	fsm_id = fsm_create_register("lte_rrc_enb_ue_fsm", rrc_main_enb_ue, sv_enb_ue);
	//初始化状态机参数
	sv_enb_ue->crnti = crnti;
	sv_enb_ue->uefsmid = fsm_id;
	return fsm_id;
}
void unregister_ue_fsm(int fsm_id)
{
	fsm_unregister_destroy(fsm_id);
}
...
/*-----------------------------------FSM creation---------------------------*/
	fsm_id = fsm_create_register("lte_rrc_enb_fsm", rrc_main_enb, priv->sv_ptr);
...
int __fsm_dev_open(struct net_device *dev)
{
	...
	//fsm_core_intf_set(STRM_TO_SRIO, DEV_INTF_REQ, DEV_PROTO_SRIO, "LTE_SRIO0");
	fsm_core_intf_set(STRM_TO_PDCP, DEV_INTF_REQ, DEV_PROTO_RRC, "LTE_PDCP0");
	fsm_core_intf_set(STRM_TO_RLCMAC, DEV_INTF_REQ, DEV_PROTO_RRC, "LTE_RLC_MAC0"); 	//add in 20150804
