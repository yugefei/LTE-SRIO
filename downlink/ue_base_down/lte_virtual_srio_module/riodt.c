...
struct rio_dma_para timer1_para = {0,0x96,0xfc100600,0x0c0efa00,0x8FC0,0,1000000,0x0,0};
struct new_check_para timer2_para;
unsigned int sendcount = 0;
unsigned int totalcount = 0;
unsigned int offset = 0x8FC0;
...
static void send_uedl_type1(struct riodt *timer_dev, u16 timer_destid, u64 timer_data, unsigned int timer_len, u64 timer_addr, void *timer_arg);
static void send_uedl_type3(struct riodt *timer_dev, u16 timer_destid, u64 timer_data, unsigned int timer_len, u64 timer_addr, void *timer_arg);
static void uedl_type2_read(void *checkaddr);
static void nr_flag(struct riodt *timer_dev, u16 timer_destid, u64 timer_data, unsigned int timer_len, u64 timer_addr, void *timer_arg);
...
static void read_dataflag(void *checkaddr);
static void enbul_type2_read(void *checkaddr);
...
int read_type2_mem()
{
	void *read_addr = mapaddr+ 0x800;
	unsigned int i;
	unsigned int offset = 0x2800;//每块缓存的大小，作为偏移量

	for(i = 0; i <= 5;i++)
	{
		if((*((u32 *)(read_addr + i*offset)) ==1))
		{
...
static void riodt_dbell_event(struct rio_mport *mport, void *dev_id, u16 sid, u16 tid, u16 info)
{
	...
	subframenum = (unsigned int)(info%10);
	sysframenum = (unsigned int)(info/10);
	printk("sysframenum is %d...subframenum is %d...\n",sysframenum,subframenum);
	if(subframenum == 2)
	{
			dest_addr = 0x830afa00;
	}
	else
	{
			dest_addr = 0x830b89c0;
	}

	send_uedl_type1(&g_riodt,0x76,dest_addr, 36800,0xfc200600,0);
	udelay(50);
	send_uedl_type3(&g_riodt,0x76,0x830e5880, 256, 0xfc200400,0);
	sendcount++;
	db_cnt ++;
...
static void riodt_dbell_event(struct rio_mport *mport, void *dev_id, u16 sid, u16 tid, u16 info)
{
	...
	send_uedl_type1(&g_riodt,0x96,dest_addr, 36800,0xfc200600,0);
	udelay(50);
	send_uedl_type3(&g_riodt,0x96,0x830e5880, 256, 0xfc200400,0);
	sendcount++;
	db_cnt ++;
	//tasklet_init(&dbell_tasklet, dbell_tasklet_handler, 0);
	//tasklet_schedule(&dbell_tasklet);
...
int riodt_mmap()
{
	...
	VirAddr = mapaddr+0x115000; //0xfc100000
...
extern int riodt_setup_cdev(void)
{
	...
	//InitQueue(&UnionQueue);
	//进行内存映射
	printk("In mmap\n");
	riodt_mmap();
	
	printk("hrtimer1 init\n");
	//rio_hrtimer1_init(NULL, 0x96);
...
//		printk("dest_addr = %#x\n",	dest_addr);
		send_uedl_type1(&g_riodt,timer1_para.destid,dest_addr,32,timer1_para.local,(void *)timer1_para.arg);
		udelay(100);
		send_uedl_type3(&g_riodt,timer1_para.destid,0x0c0ef800,40,0xfc100000,(void *)timer1_para.arg);
		sendcount ++;
	}
	else 
		uplinkcount ++;
...
static enum hrtimer_restart hrtimer_handler1(struct hrtimer *timer) 
{
	...
	static unsigned short buffnum = 0;
...
static enum hrtimer_restart hrtimer_handler3(struct hrtimer *timer) 
{
	...
	unsigned int offset = 0x8FC0;
	static unsigned int uplinkcount = 0;
	...
	if(subframenum == 2 || subframenum == 7)
	{
		...
		if(dest_addr >= timer1_para.dest + offset * 2)
		{
			dest_addr = timer1_para.dest; 
		}

		send_uedl_type1(&g_riodt,timer1_para.destid,dest_addr,/*timer1_para.len*/timer1_para.len,timer1_para.local,(void *)timer1_para.arg);
		udelay(100);
		send_uedl_type3(&g_riodt,timer1_para.destid,0x830e5880,/*timer1_para.len*/256,/*timer1_para.local*/0xfc200400,(void *)timer1_para.arg);
		sendcount ++;
	}
	else 
	{
...
//	printk("subframenum = %u\n",subframenum);
	...
	{
		sendcount = 0;
		printk("dest_addr = %#x\nsubframenum = %u\nsysframenum = %u\nuplinkcount = %u\nsendcount = %u\n",dest_addr,subframenum,sysframenum,uplinkcount,sendcount);
		return HRTIMER_NORESTART;
	}
	else
		return HRTIMER_RESTART;
...
static enum hrtimer_restart hrtimer_handler4(struct hrtimer *timer) 
{
	...
	*((unsigned int *)mapaddr) = 1;
	//printk("In hrtimer_handler3!\n");
...
//		printk("It is a commen subframe\n");
		...
		send_uedl_type1(&g_riodt,timer1_para.destid,dest_addr,timer1_para.len,timer1_para.local,(void *)timer1_para.arg);
		udelay(100);
		send_uedl_type3(&g_riodt,timer1_para.destid,0x8308c480,256,0xfc200000,(void *)timer1_para.arg);
		sendcount ++;
		totalcount++;
	}
	else 
...
{
	timer1_para.port = 0;
	timer1_para.destid = 0x96;
	timer1_para.local = 0xfc100600;
	timer1_para.dest = 0x0c0efa00;
	timer1_para.len = 0x8FC0;
...
static void send_dsp_route_info(struct riodt *timer_dev)
{
	...
	*/
	...
	riodt_map_write(&(timer_dev->dma_dev[1]), 0x74,0x83000400, 16, 0xfc400500, 0);
}


static void send_uedl_type1(struct riodt *timer_dev, u16 timer_destid, u64 timer_data, unsigned int timer_len, u64 timer_addr, void *timer_arg)
{
	UEPHYADPtoPHYType1 uetype1;
	
	uetype1.GHead.NewFlag = 1;//32bit
	uetype1.GHead.SFN = sysframenum;//16bit
	uetype1.GHead.SubframeN = subframenum;//16bit
	//PDCCH memory offset
	uetype1.DL_TYPE1_PUBLIC_C.PDCCHOffset = sizeof(GeneralHeader12) + sizeof(UE_DL_TYPE1_PUBLIC_C); //32bit
	//PDSCH memory offset
	uetype1.DL_TYPE1_PUBLIC_C.PDSCHOffset = sizeof(GeneralHeader12) + sizeof(UE_DL_TYPE1_PUBLIC_C) + sizeof(UE_DL_TYPE1_PDCCH_C);//32bit
	//PDCCH para
	uetype1.DL_TYPE1_PDCCH_C.PdcchNum = 1;//16bit
	uetype1.DL_TYPE1_PDCCH_C.RNTI = 61;//61~65;16bit
	//PDSCH para
	uetype1.DL_TYPE1_PDSCH_C.PA = 4;//16bit	
	uetype1.DL_TYPE1_PDSCH_C.PB = 1;//16bit	
	uetype1.DL_TYPE1_PDSCH_C.RNTI = 61;//61~65;16bit	
	uetype1.DL_TYPE1_PDSCH_C.UeCategory = 3;//16bit	
	uetype1.DL_TYPE1_PDSCH_C.DelayMod = 0;//16bit
	uetype1.DL_TYPE1_PDSCH_C.padding = 0;//16bit
	memcpy(mapaddr + 0x600,&uetype1,sizeof(UEPHYADPtoPHYType1));	
//	printk("type1size = %u\n",sizeof(UEPHYADPtoPHYType1));
	riodt_map_write(&(timer_dev->dma_dev[1]), timer_destid, timer_data, sizeof(UEPHYADPtoPHYType1), timer_addr, timer_arg);
}

static void send_uedl_type3(struct riodt *timer_dev, u16 timer_destid, u64 timer_data, unsigned int timer_len, u64 timer_addr, void *timer_arg)
{
	UEPHYADPtoPHYType3 uetype3;

	//GHead3 para
	uetype3.GHead.NewFlag = 1;//32bit
	uetype3.GHead.SetFlag.CmdFlag = 1;//32bit
	//public head
	uetype3.DL_TYPE3_PUBLIC_C.PDSCHOffset = sizeof(GeneralHeader345) + sizeof(UE_DL_TYPE3_PUBLIC_C) + sizeof(UE_DL_TYPE3_PDCCH_C);//32bit
	uetype3.DL_TYPE3_PUBLIC_C.PDCCHOffset = sizeof(GeneralHeader345) + sizeof(UE_DL_TYPE3_PUBLIC_C);//32bit
	//PDCCH_C
	if(subframenum == 1 || subframenum == 6)
		uetype3.DL_TYPE3_PDCCH_C.SFDirection = 1;//16bit
	else if(subframenum == 2 || subframenum == 7)
		uetype3.DL_TYPE3_PDCCH_C.SFDirection = 2;//16bit
	else
		uetype3.DL_TYPE3_PDCCH_C.SFDirection = 3;//16bit
	uetype3.DL_TYPE3_PDCCH_C.T_MODE = 3;//16bit
	uetype3.DL_TYPE3_PDCCH_C.PdcchNum = 1;//16bit
	uetype3.DL_TYPE3_PDCCH_C.RNTI = 61;//61~65;16bit
	uetype3.DL_TYPE3_PDCCH_C.UeCategory = 3;//16bit
	uetype3.DL_TYPE3_PDCCH_C.padding = 0;//16bit
	uetype3.DL_TYPE3_PDCCH_C.MemStart = 0xFC100800;//TYPE2 address;32bit
	uetype3.DL_TYPE3_PDCCH_C.MemSize = 0x7D000;//TYPE2 memory size;32bit
	//PDSCH_C
	uetype3.DL_TYPE3_PDSCH_C.DL_Status = 2;//16bit
	uetype3.DL_TYPE3_PDSCH_C.padding = 0;//16bit
	memcpy(mapaddr,&uetype3,sizeof(UEPHYADPtoPHYType3));
//	printk("type3size = %u\n",sizeof(UEPHYADPtoPHYType3));
	riodt_map_write(&(timer_dev->dma_dev[1]), timer_destid, timer_data, sizeof(UEPHYADPtoPHYType3), timer_addr, timer_arg);
}
/*
static void uedl_type2(struct riodt *timer_dev, u16 timer_destid, u64 timer_data, unsigned int timer_len, u64 timer_addr, void *timer_arg)
{
	...
	uetype2.GHead.SFN = sysframenum;//16bit
	uetype2.GHead.SubframeN = subframenum;//16bit
...
static void nr_flag(struct riodt *timer_dev, u16 timer_destid, u64 timer_data, unsigned int timer_len, u64 timer_addr, void *timer_arg)
{
	riodt_map_read(&(timer_dev->dma_dev[0]), timer1_para.destid,timer1_para.local,4,timer1_para.dest,(void *)timer1_para.arg);
//	printk("1:The data of %#llx is %u\n",timer1_para.dest,*((unsigned int *)timer1_para.local));
//	printk("1:The nread completed!\n");
	riodt_map_write(&(timer_dev->dma_dev[1]), timer1_para.destid,timer1_para.dest + 0x4,timer1_para.len,timer1_para.local,(void *)timer1_para.arg);
//	printk("2:The data of %#llx is %u\n",timer1_para.dest,*((unsigned int *)timer1_para.local));
//	printk("1:The nwrite completed!\n");
}

/*
static void read_dataflag(void *checkaddr)
{
	void *read_addr = mapaddr + (check_para.local - 0xfc100000);
	unsigned int flag = *((u32 *)read_addr);

	while(flag == 0)
	{
		flag = *((u32 *)read_addr);
	}
	memcpy(checkaddr,read_addr,1500);
	*((u32 *)read_addr) = 0;
	printk("I am the read timer! the flag is NEW DATA!\n");
	uedl_type2_read(checkaddr);
}
