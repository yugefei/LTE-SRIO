...
MODULE_DESCRIPTION("lterlc kernel module");
MODULE_AUTHOR("");
MODULE_LICENSE("GPL");
...
#define SV_TYPE_NAME		rlc_sv
#define MY_PROTOCOL		DEV_PROTO_RLC
//#define MTU			1500
#define RLC_ADDR		{0x00,0x11,0x1A,0x1A,0x8A,0x42}			// Six bytes
#ifdef VIRTUAL_TEST 
#define MTU 1500
#else
#define MTU 65535
...
/*-----------------------------------FSM creation-----------------------------*/
	fsm_id = fsm_create_register("lte_rlc_fsm", rlc_main, priv->sv_ptr);
...
{   
	fsm_priv *priv;
	const char  *name = "LTE_RLC%d";
	char dev_addr[6] = RLC_ADDR;
	NETDEV *dev = NULL;
...
int __fsm_dev_open(struct net_device *dev)
{
	...
	fsm_core_intf_set(STRM_TO_IPADP, DEV_INTF_IND, DEV_PROTO_IPADP, "LTE_IPADP0");
	fsm_core_intf_set(STRM_TO_MAC, DEV_INTF_REQ, DEV_PROTO_RLC, "LTE_MAC0");   ///
	fsm_core_intf_set(STRM_TO_SRIO, DEV_INTF_REQ, DEV_PROTO_RLC, "LTE_SRIO0");
	dev_add_pack(&pk_t); 
...
/******************************************************************************
 ** Function name:__fsm_dev_get_stats
 ** Descriptions:
 ** Input: 
 ** Output: 
 ** Returns: 
