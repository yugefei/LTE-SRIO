...
struct rio_dma_para timer1_para = {0,0x74,0xfc100800,0x81258000,0xfa0,0,1000000,0x0,0};
...
static enum hrtimer_restart hrtimer_handler0(struct hrtimer *timer) 
{
	...
	if(subframenum != 2 && subframenum != 7)
	{
		...
		if(Pop(&UnionQueue[subframenum], &CurPos))
		{
			...
			riodt_map_write(&(g_riodt.dma_dev[1]), 0x74, dest_addr,4000, (AddrStart[subframenum]+CurPos*0xfa0), (void *)0);
			send_enb_type3(&g_riodt,0x74,0x8125CF00,256,0xfc100600,(void *)timer1_para.arg);
			//sendcount++;
...
static void send_dsp_route_info(struct riodt *timer_dev)
{
	...
	*/
	...
	riodt_map_write(&(timer_dev->dma_dev[1]), 0x74, 0x0c0ba100, 16, 0xfc400500, 0);
}
